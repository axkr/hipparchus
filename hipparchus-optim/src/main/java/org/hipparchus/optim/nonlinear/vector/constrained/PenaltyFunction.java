 ...
 *  - Use value(alpha) to evaluate penalty at x + alpha * dx
...
//    private RealVector iqVal;
     ...
     */
    public PenaltyFunction(TwiceDifferentiableFunction objective,
                           Constraint eqConstraint,
                           Constraint iqConstraint,RealVector x) {
        this.objective = objective;
        this.eqConstraint = eqConstraint;
        this.iqConstraint = iqConstraint;
        this.x=x.copy();
        int m=0;
        if (this.eqConstraint!=null)m=m+this.eqConstraint.dimY();
        if (this.iqConstraint!=null)m=m+this.iqConstraint.dimY();
     ...
     */
    public void update(RealVector J,
                       RealMatrix JE,
                       RealMatrix JI,
                       RealVector x,
                       RealVector y,
    ...
    public double value(double alpha) {
        ...
        RealVector yAlpha = y.add(u.subtract(y).mapMultiply(alpha));
...
//            RealVector gi = iqVal.subtract(iqConstraint.getLowerBound());
     ...
     *
     * @param Hessian matrix
     * @param y multipliers
     * @@param y last estimate of multiplier
     * @param dx direction of x provided by QP solution
     * @param dy direction of y provided by QP solution
     * @param dy direction of y provided by QP solution
     * @param sigmaValue value of the additional variabile of QP solution
     * @param iterations current iteration
     */
     public void updateRj(RealMatrix H,RealVector y,RealVector dx, RealVector dy,double sigmaValue,int iterations) { //r = updateRj(currentH,dx,y,u,r,sigm);
